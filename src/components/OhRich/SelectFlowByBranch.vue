  ...
  <v-main>
    ...
    <v-container>
      ...
      <v-row>
        <v-col cols="12">
          <v-img
            src="https://www.srtforex.com/assets/images/head_logo.png"
          ></v-img>
        </v-col>
      </v-row>
      <v-row>
...
export default {
  ...
  methods: {
    ...
    connectPrintEN (item, language) {
      console.log('item', item, this.beforQueue)
      // eslint-disable-next-line no-undef
      if (KBBluetooth.isConnected()) {
    ...
    async getDataFlow () {
      this.flowData = []
      await axios
        .get(this.DNS_IP + '/flow/get?shopId=' + this.shopId + '&masBranchID=' + this.masBranchID)
        .then(response => {
    ...
    gotoBooking (flowId) {
      ...
      let dataBooking = [
        {'masBranchID': this.masBranchID, 'bookingFieldId': 1664, 'flowId': flowId, 'packageId': '', 'tokenPackage': '', 'fieldId': 5235, 'fieldName': 'สกุลเงินที่ต้องการแลกเปลี่ยน', 'fieldValue': '', 'shopId': this.shopId, 'dueDate': moment().format('YYYY-MM-DD'), 'sourceLink': 'ohrich', 'userId': 'No Line', 'limitBookingCheck': 'False', 'limitBookingId': '', 'limitBookingCount': null, 'getLimitBooking': 0, 'statusUpload1': 'False', 'statusUpload2': 'False', 'fileUpload1': '', 'fileUpload2': '', 'dateSelect': moment().format('YYYY-MM-DD'), 'storeFrontCheck': 'True', 'statusBookingForm': 'BookingForm', 'bookingLanguage': this.languageSelect},
        {'masBranchID': this.masBranchID, 'bookingFieldId': 1664, 'flowId': flowId, 'packageId': '', 'tokenPackage': '', 'fieldId': 5230, 'fieldName': 'เบอร์โทร', 'fieldValue': '', 'shopId': this.shopId, 'dueDate': moment().format('YYYY-MM-DD'), 'sourceLink': 'ohrich', 'userId': 'No Line', 'limitBookingCheck': 'False', 'limitBookingId': '', 'limitBookingCount': null, 'getLimitBooking': 0, 'statusUpload1': 'False', 'statusUpload2': 'False', 'fileUpload1': '', 'fileUpload2': '', 'dateSelect': moment().format('YYYY-MM-DD'), 'storeFrontCheck': 'True', 'statusBookingForm': 'BookingForm', 'bookingLanguage': this.languageSelect}
      ]
      axios
        .post(this.DNS_IP + '/Booking/addStoreFront', dataBooking)
        .then(async response => {
          if (response.data.message === 'LimitBooking_Full') {
            this.$swal('คิวเต็มแล้ว', 'กรุณาเลือกวันที่และเวลาใหม่อีกครั้ง', 'error')
            this.overlay = false
          } else {
            // let bookNo = response.data.bookData.bookNo
            this.beforQueue = response.data.beforQueue
            // let dtt = {
            //   bookNo: bookNo,
            ...
            this.overlay = false
            await this.connectPrintEN(response.data.bookData, this.languageSelect === 0 ? 'th' : 'en')
      ...
      await axios
        ...
        .then(async response => {
          ...
          if (rs.length > 0) {
            ...
            this.connectPrintEN(rs[0], this.languageSelect === 0 ? 'th' : 'en')
          }
        }).catch(function (error) {
