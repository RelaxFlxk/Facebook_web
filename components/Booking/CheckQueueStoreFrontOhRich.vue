<template>
  <v-main>
    ...
    :color="DarkModeBackground"
        ...
        <v-row v-if="source === 'ohrich' && userId === 'No Line'">
          <v-col cols="12" class="pl-10 pr-10">
            ...
            <p style="color:gray;">{{ languageSelect === 0 ? "หมายเหตุ กดปุ่มนี้เพื่อรับการรับแจ้งเตือนคิวทาง LINE" : "Note, Please press this button to receive queue notifications via LINE." }}</p>
          </v-col>
          <!-- <v-col cols="6"  class="text-right pr-10 pl-1">
            <v-btn
              ...
              dark
              block
            ><v-icon left color="white">mdi-cellphone-text</v-icon> {{ languageSelect === 0 ? "เบอร์โทร" : "Phone Number" }}</v-btn>
          </v-col> -->
        </v-row>
        <!-- <v-row v-else>
         <v-col cols="12" class="text-center mt-4" v-if="dataLineConfig.checkConfig === false">
            <v-btn
              v-if="shopId"
              elevation="10"
              color="green"
        ...
        <v-col cols="12" v-else class="text-center mt-4">
          <v-btn
              v-if="shopId && shopId.includes('SD_') === false"
              elevation="10"
              color="green"
              dark
...
export default {
  ...
  methods: {
    ...
    async searchBooking () {
      if (this.validSearch === true) {
        ...
        let urlApi = this.DNS_IP +
            '/booking_view/getDataBookingOhrich?shopId=' +
            this.shopId +
            '&masBranchID=' +
            ...
            // this.flowSelect +
            '&dueDate=' +
            this.dateStart + '&storeFrontQueue=is not null&status=confirm and confirmJob'
        // '&dueDate=' +
    ...
    async getBookingQueue (item) {
      await axios
        .get(this.DNS_IP + '/booking_view/getDataBookingOhrich?shopId=' + this.shopId + '&dueDate=' + item[0].dueDateDay + '&masBranchID=' + item[0].masBranchID + '&flowId=' + item[0].flowId + '&storeFrontQueue=is not null&status=confirm')
        .then(async response => {
          let rs = response.data
          console.log('test', rs)
          if (rs.length > 0) {
    ...
    async getBookingData () {
      ...
      await axios
        ...
        .get(this.DNS_IP + '/booking_view/getDataBookingOhrich?shopId=' + this.shopId + '&bookNo=' + this.bookNo)
        .then(async response => {
          let rs = response.data
          console.log('rsUse', rs)
          if (rs.length > 0) {
            dataBookingData = rs
            if (dataBookingData.length > 0) {
              this.dataBookingData = dataBookingData
            }
    ...
    async checkMember (bookNo) {
      await axios.get(this.DNS_IP + '/booking_view/getDataBookingMemberOhrich?bookNo=' + bookNo)
        .then(async (response) => {
          let rs = response.data
